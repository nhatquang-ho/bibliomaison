...
if(isset($_POST['search_isbn']) && !empty($_POST["isbn"])){
  ...
  if (mysql_num_rows($books)>0) {
    ...
    while ($book = mysql_fetch_assoc($books)) {
      ...
      echo '<tr><td>' . $num . '</td><td>' . $isbn . '</td><td>' . $title . '</td><td>' . $category . '</td><td>' . $year . '</td><td>' . $authors . '</td><td>' . $summary . '</td><td>' . date('d-m-Y H:i', strtotime($last_modification)) . '</td>
...
elseif(isset($_POST['search_name']) && !empty($_POST["title"])){
  ...
  $books = mysql_query("SELECT num,isbn,title,category,year,authors,summary,last_modification FROM $name WHERE title LIKE '%$title_s%' ORDER BY num") or die('<script>console.log("Error SQL : ")' . mysql_error() . '</script>');
  if (mysql_num_rows($books)>0) {
    echo '<table>';
    echo '<h4><tr><th>Num</th><th>ISBN</th><th>Title</th><th>Category</th><th>Year</th><th>Authors</th><th>Summary</th><th>Last Modification</th><th>Delete</th><th>Update</th></tr></h4>';
